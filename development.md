...
## 阶段 10：文件审查、回收站与文档管理增强

阶段 10 的目标是让文档从“上传即可展示”升级为“上传后审查、删除后可恢复、列表可批量管理、详情可预览”的完整管理流程。
...
- `review_comment`：管理员打回原因或审查意见。
- `reviewed_by`：审查人。
- `reviewed_at`：审查时间。

普通用户上传文档后默认进入 `PENDING`，页面显示“审查中”；管理员上传文档默认 `APPROVED`。管理员通过 `/reviews` 查看待审查文档，可执行“通过”或“打回”。打回后文档仍归上传用户可见，并展示打回原因；普通用户编辑后会重新进入审查中。

普通文档列表只正常展示已通过文档；上传者本人和管理员可以看到审查中或已打回的文档状态。

### 回收站

`document` 表新增 `deleted_at` 字段。删除文档时不再直接物理删除，而是设置：

- `deleted = 1`
- `deleted_at = 当前时间`

每个用户进入 `/recycle-bin` 只能看到自己可管理的回收站文档；管理员可看到全部回收站文档。回收站显示删除时间和 30 天清理倒计时，支持恢复、彻底删除和一键清空。彻底删除会删除标签关联、文件任务、文档元数据和本地真实文件。

本阶段后续修正：回收站查询需要主动读取 `deleted = 1` 的文档，因此已移除 MyBatis-Plus 全局逻辑删除配置，避免框架自动追加 `deleted = 0` 条件导致回收站查不到刚删除的文档。删除文档移入回收站时，会同步移除该文档关联的文件任务，侧边栏任务角标和 `/file-tasks` 列表也会过滤已删除文档的残留任务。

回收站操作继续增强：支持勾选多个回收站文档后批量恢复或批量彻底删除，也支持“一键恢复”当前用户回收站内全部文档。“一键清空”仍用于彻底清理当前用户回收站内全部文档。
...
该接口复用文档访问权限，并以 `Content-Disposition: inline` 返回文件资源。详情页根据文件类型选择预览方式：

- PNG、JPG、JPEG：图片预览。
- PDF、TXT、CSV：iframe 预览。
- 其他 Office 类型：显示文件信息占位，并保留下载入口。
...
```sql
source docs/database/upgrade-stage10-review-recycle.sql;
```
...
- `mvn package` 构建成功。
- 浏览器中应重点验证：普通用户上传后进入审查中、管理员审查通过后正常展示、打回后显示意见、批量删除进入回收站、回收站恢复和清空、图片/PDF/TXT 预览。
